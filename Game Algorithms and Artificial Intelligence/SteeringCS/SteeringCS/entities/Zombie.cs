...
namespace SteeringCS.entities
{
    public class Zombie : Entity
    {
        ...
        public Zombie(World world, string name, Vector_2D pos, double mass, float size, Vector_2D vel = null)
        {
            MyWorld = world;
            Name = name;
            Position = pos.Clone();
        ...
        {
            using (Brush detectionBrush = new SolidBrush(Color.FromArgb(25, 180, 60, 0)))
                g.FillEllipse(detectionBrush,
                    (float)(Position.X - DetectionRadius), (float)(Position.Y - DetectionRadius),
                    DetectionRadius * 2, DetectionRadius * 2);

            using (Pen detectionPen = new Pen(Color.FromArgb(60, 180, 60, 0), 1f))

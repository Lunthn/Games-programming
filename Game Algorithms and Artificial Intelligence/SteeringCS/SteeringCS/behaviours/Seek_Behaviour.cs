...
namespace SteeringCS.behaviour
{
    ...
    class Seek_Behaviour : Steering_Behaviour
    {
        ...
        public override Vector_2D Calculate()
        {
            ...
            double force;
            if (Owner.Seek_force_like_gravity)
            {
                // --closer is bigger force (like gravity, although gravity has / distance *distance) ----
                double gravity_factor = 100000;
                force = gravity_factor / (distance);
            }
            else
            {
                // -- the further away, the bigger the force/desire to go to target ---
                force = 2 * distance; // or distance * distance; // or whatever is suitable
            }
